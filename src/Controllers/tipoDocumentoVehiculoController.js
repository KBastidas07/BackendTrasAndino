...
export const getAllTipoDocumentoVehiculo = async (req, res, next) => {
  try {
    ...
    console.log(
      `Enviando respuesta con ${tipos.length} tipos de documento de vehículo`
    );
    res.status(200).json({
      status: "success",
      count: tipos.length,
    ...
    next(
      errorTypes.ServerError(
        "Error al obtener los tipos de documento de vehículo"
      )
    );
  }
};
  ...
  try {
    ...
    if (!tipo)
      throw errorTypes.NotFoundError(
        "Tipo de documento de vehículo no encontrado"
      );
  ...
  try {
    ...
    if (!idEmpresaExterna)
      throw errorTypes.ValidationError(
        "El campo idEmpresaExterna es obligatorio"
      );

    const result = await TipoDocumentoVehiculo.create({
      nombre,
      idEmpresaExterna,
    });
...
export const updateTipoDocumentoVehiculo = async (req, res, next) => {
  try {
    ...
    const result = await TipoDocumentoVehiculo.update(req.params.id, {
      nombre,
      idEmpresaExterna,
    });
    if (result === 0)
    ...
    if (error.code === "ER_ROW_IS_REFERENCED_2") {
      next(
        errorTypes.ConflictError(
          "No se puede eliminar el tipo de documento de vehículo porque está siendo utilizado"
        )
      );
    } else {
      next(error);
