...
export const getTipoDocumentoVehiculoById = async (req, res, next) => {
  try {
    ...
    if (!tipo) throw errorTypes.NotFoundError("Tipo de documento de vehículo no encontrado");
...
export const createTipoDocumentoVehiculo = async (req, res, next) => {
  try {
    const { nombre, idEmpresaExterna } = req.body;

    // Validar campos requeridos
    if (!nombre) throw errorTypes.ValidationError("El campo nombre es requerido");
    if (!idEmpresaExterna) throw errorTypes.ValidationError("El campo idEmpresaExterna es obligatorio");

    const result = await TipoDocumentoVehiculo.create({ nombre, idEmpresaExterna });
...
export const updateTipoDocumentoVehiculo = async (req, res, next) => {
  try {
    ...
    const result = await TipoDocumentoVehiculo.update(req.params.id, { nombre, idEmpresaExterna });
    if (result === 0) throw errorTypes.NotFoundError("Tipo de documento de vehículo no encontrado");
